...
function makeData(user){
  ...
  for(const comment of user.comments){
    ...
    if(data[date]){
      ...
      data[date] = {
        date: `${date.getFullYear()}-${(date.getMonth() + 1 + '').padStart(2, '0')}-${(date.getDate() + 1 + '').padStart(2, '0')}`,
        count: 1,
        level: 1
      }
  ...
  for(let i = 0; i < 365; i++){
    ...
    const formattedDate = `${zeroDate.getFullYear()}-${(zeroDate.getMonth() + 1 + '').padStart(2, '0')}-${(zeroDate.getDate() + 1 + '').padStart(2, '0')}`
...
const ProfileContributeBar = ({id}) => {
  ...
  useEffect(() => {
    getProfile(id).then((data) => {
      const dataArray = makeData(data.user)
  ...
  return  (
    <>
    {
      ...
      :
      <ActivityCalendar data={myData} renderBlock={(block, activity) => (
        <Tooltip title={`${activity.count} активностей на ${activity.date}`} arrow>
          {block}
        </Tooltip>
      )}/>
    }
